   ...
   if (pairingCode && !XeonBotInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Enter Your WhatsApp Number With Your Contry Code, 📌Example : +263714373922")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Enter Your WhatsApp Number With Your County Code 😊\n📌Example: +263714373922 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
